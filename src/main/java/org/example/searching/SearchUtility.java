...
public class SearchUtility {
    ...
    public static int linearSearch(int[] array, int targetValue) {
        ...
        for (int i : array) {                   // O(n) -> has n number of elements
            if (i == targetValue) {
                found = targetValue;
                break;
            }
    ...
    public static int binarySearchIterative(int[] array, int targetValue) {
        ...
        while (min <= max) {
            ...
            if (array[mid] == targetValue) {
                ...
                // target is greater, ignore left half of array
                min = mid + 1;
            } else if (array[mid] > targetValue) {
                // target is smaller, ignore right half of array
                max = mid - 1;
            }
        }
        return -1;
    ...
    private static int binarySearchRecursive(int[] arr, int minIndex, int maxIndex, int targetValue) {
