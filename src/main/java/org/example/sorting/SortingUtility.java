...
public class SortingUtility {
    ...
    public static <T extends Comparable<T>> void merge(T[] data, int first, int mid, int last) {
        ...
        int first2 = mid + 1, last2 = last; // endpoints of 2nd subarray
        int index = first1;                 // next index open in temp array

        // copy smaller item from each subarray into temp until a subarray is exhausted
        ...
        // copy remaining elements from 2nd subarray, if any
        while(first2 <= last2) {
            temp[index] = data[first2];
            first2++;
            index++;
